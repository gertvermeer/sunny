...
def maxtemp():
    ...
    max_today_url = 'http://ledmachine.local:8086/query?db=weerstats&q=SELECT max(buiten) from temperatuur where time > now() - 24h'
    resp  = requests.get('http://ledmachine.local:8086/query?db=weerstats&q=SELECT top(max,5) FROM (SELECT max(buiten) from temperatuur GROUP BY time(24h))', verify=False)
    json_data = resp.json()
    ...
    response = requests.get(max_today_url).json()
    result = result + str(response.get("results",{})[0].get("series")[0].get("values")[0][1]) + " C     " +return_data(response.get("results",{})[0].get("series")[0].get("values")[0][0])  + " ;"
    return jsonify(result)

@app.route('/mintemp', methods=['GET'])
def mintemp():
    max_url= 'http://ledmachine.local:8086/query?db=weerstats&q=SELECT min(buiten)  from temperatuur where time >= \'from_time\' and time < \'to_time\''
    max_today_url = 'http://ledmachine.local:8086/query?db=weerstats&q=SELECT min(buiten) from temperatuur where time > now() - 24h'
    resp  = requests.get('http://ledmachine.local:8086/query?db=weerstats&q=SELECT bottom(min,5) FROM (SELECT min(buiten) from temperatuur GROUP BY time(24h))', verify=False)
    json_data = resp.json()
